  ...
  try {
    ...
    if (method === 'GET') {
      ...
      return {
        ...
        body: JSON.stringify({ items, offset: data.offset || null }),
      };
    }
    ...
    if (method === 'POST') {
      ...
      return {
        statusCode: 200,
        headers: { ...cors, 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: data.records[0].id, ...data.records[0].fields }),
      };
    }
    ...
    if (method === 'PATCH') {
      ...
      const res = await fetch(`${BASE_URL}/${id}`, {
        method: 'PATCH',
        headers,
        body: JSON.stringify({ typecast: true, fields: sanitizeFields(fields) }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error?.message || 'Airtable error');
      ...
      return {
        statusCode: 200,
        headers: { ...cors, 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: data.id, ...data.fields }),
      };
    }
...
function sanitizeFields(f) {
  const map = {
    ...
    season: 'Stagione',
    digital: 'Visibilit\u00e0 Digitale',
    cannibal: 'Cannibalizzazione',
    cost: 'Costo Acquisto',
    price: 'Prezzo Vendita',
    marketPrice: 'Prezzo Mercato Live',
    marketText: 'Note Mercato',
    score: 'Score Predittivo',
    year: 'Anno Periodo',
    notes: 'Note',
    status: 'Status',
  };
